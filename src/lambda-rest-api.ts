...
export interface LambdaRestApiAppOptions extends TypeScriptProjectOptions {
  /**
...
export class AwsCdkTsLambdaRestApiApp extends AwsCdkTypeScriptApp {
  constructor(options: LambdaRestApiAppOptions) {
    const readme = fs.readFileSync(path.join(__dirname, '..', 'sample_apps', 'README.md'), 'utf-8');
    super({
      ...options,
      sampleCode: false,
      readme: {
    ...
    new SampleDir(this, this.srcdir, {
      sourceDir: path.join(__dirname, '..', 'sample_apps', versionDir, 'lambda-rest-api', 'src'),
    });
  }
}